...
export function checkStatus(user: User): void {
  if (user.status.code === 'INACTIVE') throw new UnauthorizedError(errors.USER_INACTIVE);
  if (!user.registrationConfirmed) throw new UnauthorizedError(errors.USER_UNCONFIRMED);
}
